-- Veritabanının olup olmadığını kontrol et ve yoksa oluştur
DO
$$
BEGIN
    -- Veritabanı var mı diye kontrol et
    IF NOT EXISTS (SELECT 1 FROM pg_database WHERE datname = 'northwind') THEN
        -- Veritabanı yoksa oluştur
        PERFORM dblink_exec('dbname=postgres', 'CREATE DATABASE northwind');
    END IF;
END
$$
LANGUAGE plpgsql;

-- Şimdi northwind veritabanına bağlan
\c northwind;
--
